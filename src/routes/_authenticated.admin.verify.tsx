...
const KIND_LABEL: Record<string, string> = {
  ...
  static_config: "Store config",
  xpub_scan: "Key scan match",
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Paste an address, transaction hash, or xpub. We match it against every merchant key —
          ...
          {result.matches.map((m, i) => (
            <div key={i} className="rounded-md border border-border/60 bg-card p-4 text-sm">
              <div className="flex flex-wrap items-center gap-2">
                ...
                {m.chain && (
                  <span className="rounded bg-muted px-2 py-0.5 text-[11px] uppercase">{m.chain}</span>
                )}
                {m.derivation_path && (
              ...
              <dl className="mt-3 grid gap-x-6 gap-y-2 sm:grid-cols-2">
                ...
                <Row label="Owner">{m.owner_email ?? "—"}</Row>
                {m.address && <Row label="Address" mono>{m.address}</Row>}
                {m.tx_hash && <Row label="Tx hash" mono>{m.tx_hash}</Row>}
                {m.invoice_id && (
